...
`FactorData` is the internal storage for one factor. A factor owns a stable
exchange buffer, a minimization function, certainty-preservation bookkeeping,
and an enabled flag. `FactorGraph` drives this object during the factor pass.
...
`minimization_function` is the factor-specific rule. `exchanges` is one entry
per connected edge, in factor creation order. `incoming_infinite_weights`
records which incoming messages were certain so the factor pass can preserve
that certainty after the minimizer runs. `enabled` controls whether this factor
...
`EdgeData` supplies incoming and outgoing edge messages. `GraphEdge` identifies
which edge each exchange entry belongs to. The weighted-value types define the
message payload. `RngCore` is the object-safe random-number interface passed
through to minimizers.
...
The closure field prevents deriving `Debug`, because function trait objects do
not expose a useful debug representation. The fields remain private to keep
factor-pass sequencing under `FactorData`'s control.
...
The manual implementation shows inspectable state and intentionally omits the
minimizer. `finish_non_exhaustive()` makes that omission explicit.

### Constructor And State Methods
...
Construction copies the edge handles into exchange entries. That fixes the
order in which the minimizer sees its variables.
...
Enable and disable report whether anything changed so `FactorGraph` can avoid
repeating edge bookkeeping. The exchange accessor is read-only; callers can see
which edges belong to the factor without mutating the buffer.
...
The factor pass has three stages. First it copies incoming variable-to-factor
messages into the exchange buffer and remembers which were infinite. Then it
calls the minimizer. Finally it writes the minimizer's outputs back to the
edges, forcing any previously infinite incoming message to remain infinite in
...
`edge_index` centralizes validation before indexing into edge storage.
`switch_enabled` is the shared state transition for `enable` and `disable`.
...
The tests build small edge vectors directly, since `FactorData` is an internal
type and does not need to go through the public graph API.
...
This verifies both the factor's enabled state and the boolean return values
from repeated state transitions.
...
The factor is created with edge order `[1, 0]` to prove that exchange order is
factor-local, not sorted by edge index.
...
This is the direct test for certainty preservation. The minimizer attempts to
downgrade both weights to zero; only the edge with infinite incoming weight is
forced back to infinite.
...
A disabled factor must be a no-op, including not running its minimizer.

## Important Invariants

- Exchange order is the order supplied at factor creation.
- Infinite incoming weight is preserved on the outgoing factor message.
- Disabled factors do not inspect or mutate edge state.

## Algorithm Context

`FactorData` is the factor-pass half of each TWA iteration. It translates edge
messages into minimizer input, runs the constraint-specific minimizer, and
translates the result back into edge state for the variable pass.

## Extension Notes

Future built-in minimizers should rely only on the exchange API exposed here:
read with `get`, write with `set`, and use the edge handle only for identity.
